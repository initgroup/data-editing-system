...
CREATE TABLE "INIT$_TB_REPORT_TMPL_USE" (
  ...
  , CONSTRAINT "INIT$_CK_RPT_USE_COUNT" CHECK ("USE_COUNT" >= 1) ENABLE
    ...
    create_index_if_missing(
        ...
        'INIT$_TB_REPORT_TEMPLATE',
        'CREATE INDEX "INIT$_IX_CUSTOM_REPORT_USER" ON "INIT$_TB_REPORT_TEMPLATE" ("USER_ID", "USE_YN", "UPDATED_AT")'
    );

    create_index_if_missing(
        'INIT$_IX_RPT_USE_PROJECT',
        'INIT$_TB_REPORT_TMPL_USE',
        'CREATE INDEX "INIT$_IX_RPT_USE_PROJECT" ON "INIT$_TB_REPORT_TMPL_USE" ("USER_ID", "PROJECT_ID", "LAST_USED_AT")'
    );
    ...
    add_column_if_missing('INIT$_TB_TABLES', 'ORIGINAL_OWNER_NAME', '"ORIGINAL_OWNER_NAME" VARCHAR2(128 BYTE)');
    add_column_if_missing('INIT$_TB_TABLES', 'ORIGINAL_TABLE_NAME', '"ORIGINAL_TABLE_NAME" VARCHAR2(128 BYTE)');
    add_column_if_missing('INIT$_TB_TABLES', 'EDIT_OWNER_NAME', '"EDIT_OWNER_NAME" VARCHAR2(128 BYTE)');
    add_column_if_missing('INIT$_TB_TABLES', 'EDIT_TABLE_NAME', '"EDIT_TABLE_NAME" VARCHAR2(128 BYTE)');
    add_column_if_missing('INIT$_TB_TABLES', 'DATA_ORIGIN_TYPE', '"DATA_ORIGIN_TYPE" VARCHAR2(30 BYTE) DEFAULT ''MANAGED_TABLE''');
    add_column_if_missing('INIT$_TB_TABLES', 'CASE_ID_COLUMN', '"CASE_ID_COLUMN" VARCHAR2(128 BYTE) DEFAULT ''FILE_ROW_NO''');
    add_column_if_missing('INIT$_TB_TABLES', 'IMPORTED_AT', '"IMPORTED_AT" TIMESTAMP (6)');

    IF table_exists('INIT$_TB_TABLES')
       AND column_exists('INIT$_TB_TABLES', 'EDIT_TABLE_NAME') THEN
        run_ddl(
...
UPDATE "INIT$_TB_TABLES"
 ...
 WHERE "ORIGINAL_OWNER_NAME" IS NULL
    ...
    END IF;
    create_index_if_missing('IX_INIT$_TB_TABLES_MAP_01', 'INIT$_TB_TABLES', q'[
CREATE INDEX "IX_INIT$_TB_TABLES_MAP_01"
    ON "INIT$_TB_TABLES" ("PROJECT_ID", "SCENARIO_ID", "ORIGINAL_OWNER_NAME", "ORIGINAL_TABLE_NAME")
]');

    create_index_if_missing('IX_INIT$_TB_TABLES_REG_01', 'INIT$_TB_TABLES', q'[
...
CREATE TABLE "INIT$_TB_UPLOAD_TABLE_META" (
    ...
    create_index_if_missing('IX_INIT$_TB_UPLOAD_META_01', 'INIT$_TB_UPLOAD_TABLE_META', q'[
...
CREATE INDEX "IX_INIT$_TB_RULEDISC_ASSOC_SUM_03"
    ...
    create_index_if_missing('IX_INIT$_TB_RULEDISC_ASSOC_SUM_04', 'INIT$_TB_RULEDISC_ASSOC_SUM', q'[
...
CREATE INDEX "IX_INIT$_TB_RULEDISC_SYMBOLIC_01"
    ...
    create_index_if_missing('IX_INIT$_TB_RULEDISC_SYMBOLIC_02', 'INIT$_TB_RULEDISC_SYMBOLIC', q'[
...
-- Editing lifecycle objects are maintained in a separate block so this
-- migration remains independently rerunnable after the cleanup above.
DECLARE
    v_unresolved_edit_rule_count NUMBER := 0;
        ...
        ELSE
            DBMS_OUTPUT.PUT_LINE(
                '[SKIP] COLUMN ' || p_table_name || '.' || p_column_name
                    || ' already exists or table is missing.'
            );
        END IF;
    END;
        ...
        IF v_nullable = 'Y' THEN
            EXECUTE IMMEDIATE
                'ALTER TABLE "' || UPPER(p_table_name) || '" MODIFY ("'
                    || UPPER(p_column_name) || '" NOT NULL ENABLE)';
            DBMS_OUTPUT.PUT_LINE(
                '[OK] ALTER ' || p_table_name || '.' || p_column_name || ' NOT NULL'
            );
        ELSE
            DBMS_OUTPUT.PUT_LINE(
                '[SKIP] COLUMN ' || p_table_name || '.' || p_column_name || ' is already NOT NULL.'
            );
        END IF;
    END;
BEGIN
...
CREATE TABLE "INIT$_TB_EDIT_RULE" (
    ...
    add_column_if_missing('INIT$_TB_EDIT_RULE', 'RULE_TOLERANCE_PCT', '"RULE_TOLERANCE_PCT" NUMBER(9,4) DEFAULT 5');

    IF table_exists('INIT$_TB_EDIT_RULE') THEN
        EXECUTE IMMEDIATE q'[
MERGE INTO "INIT$_TB_EDIT_RULE" E
USING (
      SELECT X.RUN_SOURCE_TYPE
           , X.RUN_ID
           , MIN(X.PROJECT_ID) AS PROJECT_ID
           , MIN(X.SCENARIO_ID) AS SCENARIO_ID
        FROM (
       ...
       GROUP BY X.RUN_SOURCE_TYPE
              , X.RUN_ID
      HAVING COUNT(DISTINCT TO_CHAR(X.PROJECT_ID) || ':' || TO_CHAR(X.SCENARIO_ID)) = 1
      ) R
   ON (
          E.SOURCE_RUN_SOURCE_TYPE = R.RUN_SOURCE_TYPE
      AND E.SOURCE_RUN_ID = R.RUN_ID
      )
 WHEN MATCHED THEN
 UPDATE SET E.PROJECT_ID = R.PROJECT_ID
          , E.SCENARIO_ID = R.SCENARIO_ID
  WHERE E.PROJECT_ID IS NULL
     OR E.SCENARIO_ID IS NULL
]';
...
USING (
      SELECT T.OWNER_NAME
           ...
           , MIN(T.SCENARIO_ID) AS SCENARIO_ID
        FROM "INIT$_TB_TABLES" T
       WHERE T.USE_YN = 'Y'
       GROUP BY T.OWNER_NAME
              , T.TABLE_NAME
      HAVING COUNT(DISTINCT TO_CHAR(T.PROJECT_ID) || ':' || TO_CHAR(T.SCENARIO_ID)) = 1
      ) T
   ON (
          E.TARGET_OWNER = T.OWNER_NAME
      AND E.TARGET_TABLE = T.TABLE_NAME
      )
 WHEN MATCHED THEN
 UPDATE SET E.PROJECT_ID = T.PROJECT_ID
          , E.SCENARIO_ID = T.SCENARIO_ID
  WHERE E.PROJECT_ID IS NULL
     OR E.SCENARIO_ID IS NULL
]';
...
]' INTO v_unresolved_edit_rule_count;

        IF v_unresolved_edit_rule_count > 0 THEN
            RAISE_APPLICATION_ERROR(
                -20051,
                'INIT$_TB_EDIT_RULE contains '
                    || v_unresolved_edit_rule_count
                    || ' row(s) whose PROJECT_ID or SCENARIO_ID cannot be resolved safely.'
            );
        END IF;

        enforce_not_null('INIT$_TB_EDIT_RULE', 'PROJECT_ID');
        enforce_not_null('INIT$_TB_EDIT_RULE', 'SCENARIO_ID');
    END IF;
...
CREATE TABLE "INIT$_TB_EDIT_SESSION" (
    ...
    add_column_if_missing('INIT$_TB_EDIT_SESSION', 'BASELINE_FLOW_RUN_ID', '"BASELINE_FLOW_RUN_ID" NUMBER');
    add_column_if_missing('INIT$_TB_EDIT_SESSION', 'REANALYSIS_FLOW_RUN_ID', '"REANALYSIS_FLOW_RUN_ID" NUMBER');
    add_column_if_missing('INIT$_TB_EDIT_SESSION', 'REANALYSIS_STATUS', '"REANALYSIS_STATUS" VARCHAR2(30 BYTE)');
...
CREATE TABLE "INIT$_TB_EDIT_EVENT" (
    ...
    create_index_if_missing('IX_INIT$_TB_EDIT_RULE_01', 'INIT$_TB_EDIT_RULE', q'[
CREATE INDEX "IX_INIT$_TB_EDIT_RULE_01"
    ...
    create_index_if_missing('IX_INIT$_TB_EDIT_RULE_02', 'INIT$_TB_EDIT_RULE', q'[
