...
SELECT ORA_HASH(Q.CASE_ROWID, 4294967295) AS VIOLATION_ID
     ...
     , SYSTIMESTAMP AS CREATE_DT
  FROM (
        SELECT /*+ NO_PARALLEL(T) */ {caseIdExpression} AS CASE_ID
             , {caseRowidExpression} AS CASE_ROWID
...
-- [MCOMMON_EDIT_SESSION_RULE_MERGE]
MERGE INTO "INIT$_TB_EDIT_SESSION_RULE" T
USING (
   ...
   ON (
      ...
      AND T.EDIT_RULE_ID = S.EDIT_RULE_ID
      )
...
UPDATE "INIT$_TB_EDIT_SESSION"
   SET SESSION_STATUS = :sessionStatus
     , VALIDATED_AT = CASE
                          WHEN :sessionStatus IN ('VALIDATED', 'APPLY_READY') THEN SYSTIMESTAMP
                          WHEN :sessionStatus IN ('DRAFT', 'EDITING') THEN NULL
                          ELSE VALIDATED_AT
                      END
...
UPDATE "INIT$_TB_EDIT_SESSION"
   SET REANALYSIS_FLOW_RUN_ID = :flowRunId
     ...
     , SESSION_STATUS = CASE
                            WHEN SESSION_STATUS IN ('VALIDATED', 'APPLY_READY') THEN 'EDITING'
                            ELSE SESSION_STATUS
                        END
     , VALIDATED_AT = NULL
     , UPDATED_AT = SYSTIMESTAMP
...
SELECT Q.EVENT_DETAIL_JSON
     , Q.CREATED_AT AS VALIDATION_SNAPSHOT_AT
  FROM (
