...
BEGIN
    DELETE /*+ NO_PARALLEL */ FROM "INIT$_TB_RULEVIOL_SYMBOLIC"
     WHERE RUN_SOURCE_TYPE = :runSourceType
       AND RUN_ID = :runId
       AND TARGET_OWNER = :owner
       AND TARGET_TABLE = :tableName;

    DELETE /*+ NO_PARALLEL */ FROM "INIT$_TB_RULEVIOL_ASSOC"
     WHERE RUN_SOURCE_TYPE = :runSourceType
       AND RUN_ID = :runId
       AND TARGET_OWNER = :owner
       AND TARGET_TABLE = :tableName;
    ...
    DELETE /*+ NO_PARALLEL */ FROM "INIT$_TB_COLREL_LASSO_FEATURE"
     WHERE RUN_SOURCE_TYPE = :runSourceType
       AND RUN_ID = :runId
       AND OWNER = :owner
       AND TABLE_NAME = :tableName;
