...
SELECT COUNT(*) AS HAS_RESULT_KIND
  FROM USER_TAB_COLUMNS
 WHERE TABLE_NAME = 'INIT$_TB_RULEDISC_ASSOC_SUM'
   AND COLUMN_NAME = 'RESULT_KIND'
;

-- [REPORT_FORMULA_KIND_CURRENT]
CASE
    WHEN R.RESULT_KIND = 'FORMULA' THEN 1
    WHEN R.RESULT_KIND IS NULL
         AND R.RULE_SOURCE = 'MIXED_PATTERN_TREE'
         AND SUBSTR(R.RULE_ID, 1, 14) = 'MIXED_FORMULA_' THEN 1
    ELSE 0
END
;

-- [REPORT_FORMULA_KIND_LEGACY]
CASE
    WHEN R.RULE_SOURCE = 'MIXED_PATTERN_TREE'
         AND SUBSTR(R.RULE_ID, 1, 14) = 'MIXED_FORMULA_' THEN 1
    ELSE 0
END
;
...
SELECT *
  ...
  (
   SELECT R.OWNER
        ...
        , R.FEATURE_COLUMNS
        , R.METHOD
        , 'SYMBOLIC' AS RULE_SOURCE
        , CAST(NULL AS NUMBER) AS RULE_CONFIDENCE
     FROM INIT$_TB_RULEDISC_SYMBOLIC R
    WHERE R.RUN_SOURCE_TYPE = 'FLOW_WORK'
      AND R.RUN_ID = :flowRunId
    ORDER BY R.TARGET_COLUMN
           , R.SELECTED_YN DESC NULLS LAST
           , R.RANK_NO NULLS LAST
...
SELECT *
  FROM
  (
   SELECT R.TARGET_OWNER AS OWNER
        , R.TARGET_TABLE AS TABLE_NAME
        , R.RESULT_COLUMN AS TARGET_COLUMN
        , R.RULE_ID
        , R.RESULT_TEXT AS EXPRESSION
        , CAST(NULL AS NUMBER) AS SCORE
        , CAST(NULL AS NUMBER) AS COMPLEXITY
        , CAST(NULL AS NUMBER) AS RANK_NO
        , CAST(NULL AS CHAR(1)) AS SELECTED_YN
        , CAST(NULL AS VARCHAR2(4000)) AS FEATURE_COLUMNS
        , 'MIXED_PATTERN_FORMULA' AS METHOD
        , R.RULE_SOURCE
        , CASE
              WHEN R.RULE_CONFIDENCE BETWEEN 0 AND 1 THEN R.RULE_CONFIDENCE
              WHEN R.RULE_CONFIDENCE > 1 AND R.RULE_CONFIDENCE <= 100 THEN R.RULE_CONFIDENCE / 100
...
SELECT COUNT(*) AS SYMBOLIC_RULE_COUNT
     ...
     , SUM(R.MIXED_FORMULA) AS MIXED_FORMULA_COUNT
     , (
        SELECT COUNT(*)
          FROM
          (
           SELECT DISTINCT F.OWNER
                , F.TABLE_NAME
                , F.TARGET_COLUMN
             FROM FORMULA_RULES F
          )
       ) AS TARGET_COUNT
  FROM FORMULA_RULES R
;
...
SELECT V.VIOLATION_TYPE
     ...
     , ROUND(AVG(V.VIOLATION_SCORE), 6) AS AVG_VIOLATION_SCORE
        ...
        SELECT CASE
                   WHEN EXISTS
                        (
                         SELECT 1
                           FROM INIT$_TB_RULEDISC_ASSOC_SUM R
                          WHERE R.RUN_SOURCE_TYPE = A.RUN_SOURCE_TYPE
                            AND R.RUN_ID = A.RUN_ID
                            AND R.OWNER = A.RULE_OWNER
                            AND R.MODEL_NAME = A.MODEL_NAME
                            AND R.RULE_ID = A.RULE_ID
                            AND R.TARGET_OWNER = A.TARGET_OWNER
                            AND R.TARGET_TABLE = A.TARGET_TABLE
                            AND (/* --DYNAMIC_COLUMNS-- */) = 1
                        ) THEN 'SYMBOLIC'
                   ELSE 'ASSOCIATION'
