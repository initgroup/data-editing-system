...
class QuickEditHistoryTests(unittest.TestCase):
    ...
    def test_history_list_sql_uses_only_saved_summary_rows(self):
        ...
        self.assertIn("PROJECT_TABLE_SCOPE", sql)
        self.assertIn("PR.PROJECT_NAME", sql)
        self.assertIn("QS.PROJECT_NAME AS RUN_PROJECT_NAME", sql)
        self.assertIn("COALESCE(CT.TABLE_NAME, VT.TABLE_NAME", sql)
        self.assertIn("PTS.PROJECT_ID IS NULL THEN QS.TABLE_NAME", sql)
        self.assertNotIn("COALESCE(QS.PROJECT_NAME, PR.PROJECT_NAME) AS PROJECT_NAME", sql)
