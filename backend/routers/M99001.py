...
def _get_connection_detail(conn, connection_id: int, user_id: Optional[int] = None, include_shared: bool = True) -> dict:
    cursor = conn.cursor()
    try:
        if user_id is None:
            cursor.execute(
                ...
                SELECT
                    C.CONNECTION_ID,
                    C.USER_ID,
                    U.ROLE_CODE AS OWNER_ROLE_CODE,
                    'PRIVATE' AS CONNECTION_SCOPE,
                    C.CONNECTION_NAME,
                    C.DB_TYPE,
                    C.HOST,
                    C.PORT,
            ...
            cursor.execute(
                ...
                """,
                {
                    "connectionId": connection_id,
                    "userId": user_id,
                    "includeShared": "Y" if include_shared else "N",
                },
            )
        row = cursor.fetchone()
        if not row:
...
def _get_owned_connection_detail(conn, connection_id: int, user_id: int) -> dict:
    return _get_connection_detail(conn, connection_id, user_id, include_shared=False)
...
def _connection_row_to_params(row: dict) -> dict:
    return {
        ...
        "connectionScope": row.get("CONNECTION_SCOPE") or "PRIVATE",
        "ownerRoleCode": row.get("OWNER_ROLE_CODE"),
...
def get_connection(request: Request, connectionId: int = Query(...)):
    ...
    try:
        conn = get_db_connection()
        row = _get_owned_connection_detail(conn, connectionId, user_id)
...
def save_connection(req: ConnectionRequest, request: Request):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        existing = _get_owned_connection_detail(conn, int(req.connectionId), user_id) if req.connectionId else {}
        params = _normalize_connection(req, existing, user_id)
...
def test_connection(req: ConnectionRequest, request: Request):
    ...
    try:
        system_conn = get_db_connection()
        existing = _get_owned_connection_detail(system_conn, int(req.connectionId), user_id) if req.connectionId and user_id else {}
        params = _normalize_connection(req, existing, user_id)
        target_payload = {**params, "password": _decode_secret(params["passwordEnc"])}
