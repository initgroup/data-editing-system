    ...
    const MCOMMON = {
        ...
        createDataWorkPage(config) {
            ...
            const PARAMETER_DESCRIPTION_MESSAGE_KEYS = {
                ...
                P_SIMPLE_ARITHMETIC_FIRST_YN: "paramDescSimpleArithmeticFirstYn",
                P_SIMPLE_ARITHMETIC_MIN_MATCH_RATE: "paramDescSimpleArithmeticMinMatchRate",
                P_SIMPLE_ARITHMETIC_MAX_TERMS: "paramDescSimpleArithmeticMaxTerms",
                P_SIMPLE_ARITHMETIC_TOLERANCE_PCT: "paramDescSimpleArithmeticTolerancePct",
        ...
        enrichCurrentJobExecutionMetadata() {
            ...
            if (sourceType === "WEB_API") {
                if (this.getSavedBuiltinWebApiDefinition(this.currentJob.execMethod)) return;
        ...
        renderWebApiResources() {
            ...
            const savedBuiltin = this.getSavedBuiltinWebApiDefinition(this.currentJob?.execMethod);
            select.innerHTML = `
                <option value="">${this.escapeHtml(this.getLabel("selectPythonApiResource") || "-- Select Python API resource --")}</option>
                ${savedBuiltin ? `<option value="${this.escapeHtml(savedBuiltin.method)}" data-saved-builtin="true">${this.escapeHtml(savedBuiltin.label)} / ${this.escapeHtml(savedBuiltin.method)}</option>` : ""}
                ${resources.map((resource) => {
            ...
            const current = String(this.currentJob?.execSourceType || "").toUpperCase() === "WEB_API"
                ? (savedBuiltin?.method || this.currentJob?.execResourceId || this.findWebApiResource(this.currentJob?.execMethod)?.OML_RESOURCE_ID || "")
                : "";
        ...
        getSavedBuiltinWebApiDefinition(value) {
            ...
            const methods = {
                ...
                MIXED_XAI_RULE_DETECT: "/api/mlAnalysis/mixed-xai-rule-detect"
            };
            const method = String(saved.execMethod || "").toUpperCase();
            const requested = String(value || "").trim().toUpperCase();
            if (!Object.prototype.hasOwnProperty.call(methods, method) || requested !== method
                || String(current.execMethod || "").toUpperCase() !== method
                || String(current.execSpecJson || "") !== String(saved.execSpecJson || "")) return null;
            const spec = this.parseSpecJson(saved.execSpecJson);
            if (String(spec.adapter || "").toUpperCase() !== "INTERNAL_PYTHON_API"
                || String(spec.method || "").toUpperCase() !== method
                || (spec.endpoint || spec.serviceUrl) !== methods[method]
                || (spec.endpoint && spec.endpoint !== methods[method])
                || (spec.serviceUrl && spec.serviceUrl !== methods[method])) return null;
            return {
                savedBuiltin: true,
                resourceId: "",
                method,
                label: current.execObjectLabel || saved.execObjectLabel || saved.execObjectName || method,
            ...
            const definitions = {
                ...
                SYMBOLIC_REGRESSION_RULE: {
                    ...
                    params: [
                        ...
                        { itemName: "P_SIMPLE_ARITHMETIC_FIRST_YN", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescSimpleArithmeticFirstYn", "Try coefficient-free arithmetic rules before fitted regression (Y/N)"), itemDefault: "Y" },
                        { itemName: "P_SIMPLE_ARITHMETIC_MIN_MATCH_RATE", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescSimpleArithmeticMinMatchRate", "Minimum dominant-row match rate for a coefficient-free arithmetic rule"), itemDefault: "0.90" },
                        { itemName: "P_SIMPLE_ARITHMETIC_MAX_TERMS", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescSimpleArithmeticMaxTerms", "Maximum source term count for coefficient-free arithmetic candidates (2-4)"), itemDefault: "3" },
                        { itemName: "P_SIMPLE_ARITHMETIC_TOLERANCE_PCT", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescSimpleArithmeticTolerancePct", "Relative match tolerance for arithmetic discovery; 0.01 means 1%"), itemDefault: "0" },
                ...
                INTEGRATED_RULE_DISCOVER: {
                    ...
                    params: [
                        ...
                        { itemName: "P_MAX_RULE_LENGTH", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescMaxRuleLength", "Maximum Apriori rule length"), itemDefault: "3" },
                        { itemName: "P_DROP_EXISTING_YN", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescDropExistingYn", "Drop an existing association model before rebuilding (Y/N)"), itemDefault: "Y" },
                        { itemName: "P_MAX_INPUT_ROWS", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescMaxInputRows", "Maximum input rows for categorical rule discovery"), itemDefault: "100000" },
                ...
                INTEGRATED_RULE_VIOLATION_DETECT: {
                    ...
                    params: [
                        ...
                        { itemName: "P_TARGET_TABLE", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescTargetTable", "Target table name"), itemDefault: targetTable },
                        { itemName: "P_CASE_ID_COLUMN_NAME", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescCaseIdColumn", "Case ID column used to identify source rows"), itemDefault: "FILE_ROW_NO" },
                        { itemName: "P_CAT_RESULT_OWNER", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescCatResultOwner", "Owner for categorical violation results"), itemDefault: targetOwner },
                        { itemName: "P_CAT_RESULT_TABLE", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescCatResultTable", "Categorical violation result table"), itemDefault: "INIT$_TB_RULEVIOL_ASSOC" },
                        { itemName: "P_SYMBOLIC_RESULT_OWNER", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescSymbolicResultOwner", "Owner for symbolic violation results"), itemDefault: targetOwner },
                        { itemName: "P_SYMBOLIC_RESULT_TABLE", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescSymbolicResultTable", "Symbolic violation result table"), itemDefault: "INIT$_TB_RULEVIOL_SYMBOLIC" },
                        ...
                        { itemName: "P_ABS_ERROR_THRESHOLD", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescAbsErrorThreshold", "Optional absolute error threshold"), itemDefault: "(auto)" },
                        ...
                        { itemName: "P_MAX_ELAPSED_SECONDS", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescMaxElapsedSeconds", "Maximum execution time in seconds"), itemDefault: "1800" },
                        { itemName: "P_MAX_EXPRESSION_LENGTH", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescMaxExpressionLength", "Maximum symbolic expression length"), itemDefault: "8000" },
                        ...
                        { itemName: "P_COMMIT_INTERVAL", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescCommitInterval", "Rows processed between commits when commit is enabled"), itemDefault: "5000" },
                        { itemName: "P_CONTINUE_ON_ERROR", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescContinueOnError", "Continue when some automatic targets fail"), itemDefault: "Y" },
                        { itemName: "P_RUN_SOURCE_TYPE", itemValue: "VARCHAR2", itemDesc: this.getMessage("paramDescRunSourceType", "Run source type (DATA_WORK/FLOW_WORK)"), itemDefault: runSourceType },
                        { itemName: "P_RUN_ID", itemValue: "NUMBER", itemDesc: this.getMessage("paramDescRunId", "Shared DATA_WORK run ID for the scenario"), itemDefault: runId }
                    ]
...
P_PREDICTION_METHOD  =&gt; :pPredictionMethod</code></pre>
        ...
        getJobPayload(status, message) {
            ...
            return {
                ...
                execResourceId: isWebApi ? (webApi?.savedBuiltin ? null : (webApi?.resourceId || webApiResource?.OML_RESOURCE_ID || null)) : (isOml ? (omlResource?.OML_RESOURCE_ID || null) : null),
