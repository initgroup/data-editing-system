(function() {
    ...
    function ensureRoot() {
        ...
        root.innerHTML = `
            ...
            <section class="descriptive-statistics-dialog" role="dialog" aria-modal="true" aria-labelledby="descriptiveStatisticsTitle" tabindex="-1">
                ...
                <div class="descriptive-statistics-toolbar">
                    <div class="descriptive-statistics-view-control">
                        <div class="descriptive-statistics-view-heading">
                            <label for="descriptiveStatisticsColumn">분석 보기</label>
                            <button id="descriptiveStatisticsOverviewButton" type="button" hidden aria-label="전체 컬럼 중요도 요약으로 돌아가기" title="전체 요약으로 돌아가기">
                                <i class="fas fa-table-cells-large" aria-hidden="true"></i>
                                <span>전체 요약</span>
                            </button>
                        </div>
                        <div id="descriptiveStatisticsSelectors" class="descriptive-statistics-selectors">
                            <select id="descriptiveStatisticsColumn" disabled></select>
                            <select id="descriptiveStatisticsTypeFilter" aria-label="컬럼 유형 필터" hidden disabled>
        ...
        root.querySelector("#descriptiveStatisticsColumn")?.addEventListener("change", (event) => {
            state.selectedColumnName = String(event.target.value || "");
            if (state.selectedColumnName !== OVERVIEW_VALUE) activateView("detail");
            else activateView("overview");
            const content = state.root?.querySelector("#descriptiveStatisticsContent");
            if (content) content.scrollTop = 0;
        });
        root.querySelector("#descriptiveStatisticsTypeFilter")?.addEventListener("change", (event) => {
            state.detailTypeFilter = String(event.target.value || "ALL").toUpperCase();
        ...
        root.querySelector("#descriptiveStatisticsOverviewButton")?.addEventListener("click", showOverview);
    ...
    function setLoading() {
        ...
        const select = root.querySelector("#descriptiveStatisticsColumn");
        if (select) {
            select.disabled = true;
            select.innerHTML = "<option>불러오는 중...</option>";
        }
        const typeFilter = root.querySelector("#descriptiveStatisticsTypeFilter");
        if (typeFilter) {
        ...
        const overviewButton = root.querySelector("#descriptiveStatisticsOverviewButton");
    ...
    function setState(kind, title, message) {
        ...
        if (select) {
            select.disabled = true;
            select.innerHTML = "<option>선택 가능한 컬럼 없음</option>";
        }
        const typeFilter = root.querySelector("#descriptiveStatisticsTypeFilter");
        if (typeFilter) {
            typeFilter.hidden = true;
    ...
    function renderSources() {
        ...
        holder.innerHTML = sources.map((source, index) => {
    ...
    function renderTypeFilter() {
        const select = state.root?.querySelector("#descriptiveStatisticsTypeFilter");
        ...
        select.innerHTML = options.map(([value, label, count]) => (
            `<option value="${value}">${label} · ${count}개</option>`
        )).join("");
        if (!options.some(([value]) => value === state.detailTypeFilter)) state.detailTypeFilter = "ALL";
        select.value = state.detailTypeFilter;
        select.disabled = false;
    }

    function renderColumnSelector() {
        const select = state.root.querySelector("#descriptiveStatisticsColumn");
        if (!select || !state.data) return;
        select.innerHTML = "";
        const overviewOption = document.createElement("option");
        overviewOption.value = OVERVIEW_VALUE;
        overviewOption.textContent = "전체 컬럼 · 중요도 분석 보드";
        select.appendChild(overviewOption);
        const detailColumns = getDetailColumns();
        detailColumns.forEach((column) => {
            const option = document.createElement("option");
            option.value = column.columnName;
            option.textContent = column.columnComment
                ? `${column.columnName} · ${column.columnComment}`
                : column.columnName;
            select.appendChild(option);
        });
        const exists = state.selectedColumnName === OVERVIEW_VALUE
            || detailColumns.some((column) => column.columnName === state.selectedColumnName);
        if (!exists && state.selectedColumnName) state.selectedColumnName = OVERVIEW_VALUE;
        select.value = state.selectedColumnName;
        select.disabled = false;
        renderTypeFilter();
        updateOverviewNavigation();
    }

    function updateOverviewNavigation() {
        const button = state.root?.querySelector("#descriptiveStatisticsOverviewButton");
        if (button) button.hidden = !state.data || state.selectedColumnName === OVERVIEW_VALUE;
        const filter = state.root?.querySelector("#descriptiveStatisticsTypeFilter");
        const selectors = state.root?.querySelector("#descriptiveStatisticsSelectors");
        const isDetail = Boolean(state.data && state.selectedColumnName !== OVERVIEW_VALUE);
        if (filter) filter.hidden = !isDetail;
        selectors?.classList.toggle("has-type-filter", isDetail);
    }
    ...
    function showOverview() {
        ...
        const select = state.root?.querySelector("#descriptiveStatisticsColumn");
        if (select) select.value = OVERVIEW_VALUE;
        renderSelectedColumn();
    ...
    function activateView(view) {
        ...
        if (nextView === "overview") {
            ...
            const select = state.root?.querySelector("#descriptiveStatisticsColumn");
            if (select) select.value = state.selectedColumnName;
            renderSelectedColumn();
        }
        state.root?.querySelectorAll("[data-statistics-view]").forEach((button) => {
    ...
    function renderOverview() {
                ...
                <section class="descriptive-change-map" aria-labelledby="descriptiveChangeMapTitle">
                    ...
                    <div class="descriptive-change-map-scroll">
                        <table>
                            ...
                            <tbody>${visible.map(renderChangeMapRow).join("")}</tbody>
                        </table>
                    </div>
                </section>
                <div class="descriptive-insight-list-heading">
                    <div><b>${escapeHtml(formatNumber(visible.length))}개 컬럼</b><span>중요도 높은 순</span></div>
                    <p>카드를 선택하면 8개 통계값과 상세 분포 비교를 확인할 수 있습니다.</p>
                </div>
                <div class="descriptive-insight-grid">
                    ${visible.length ? visible.map(renderInsightCard).join("") : `<div class="descriptive-insight-empty">선택한 조건에 해당하는 컬럼이 없습니다.</div>`}
                </div>
        ...
        content.querySelectorAll("[data-insight-column]:not([disabled])").forEach((button) => {
            button.addEventListener("click", () => {
                state.detailTypeFilter = "ALL";
                state.selectedColumnName = button.dataset.insightColumn || OVERVIEW_VALUE;
                renderColumnSelector();
                const select = state.root.querySelector("#descriptiveStatisticsColumn");
                if (select) select.value = state.selectedColumnName;
                activateView("detail");
        ...
        return `
            <section class="descriptive-statistics-card descriptive-distribution-overlay" aria-labelledby="descriptiveDistributionOverlayTitle">
                <div class="descriptive-statistics-card-title">
                    ...
                    <div class="descriptive-distribution-legend"><span class="is-before">원본</span>${hasAfter ? `<span class="is-after">수정</span>` : ""}</div>
                </div>
                <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${escapeHtml(column.columnName)} 원본${hasAfter ? "과 수정" : ""} 데이터의 12개 동일 구간 분포">
                    <line x1="${padding}" y1="${baseline}" x2="${width - padding}" y2="${baseline}" class="is-axis"></line>
                    <path d="${beforeArea}" class="is-before-area"></path><path d="${beforePath}" class="is-before-line"></path>
                    ${hasAfter ? `<path d="${afterArea}" class="is-after-area"></path><path d="${afterPath}" class="is-after-line"></path>` : ""}
                </svg>
                ...
                <p class="descriptive-statistics-chart-note">원본과 수정 데이터를 공통 최소·최대 범위로 나눠 같은 축에서 비교합니다. 원시 값은 표시하지 않습니다.</p>
            </section>
        `;
    }
    ...
    function renderDetailColumnList(selectedColumnName) {
        const columns = getDetailColumns();
        return `<aside class="descriptive-column-rail"><header><b>컬럼 · 변화 큰 순</b><small>${columns.length}개</small></header><div>${columns.map((item) => {
            const score = item.insight?.importanceScore || 0;
            const level = score >= 70 ? "high" : (score >= 30 ? "medium" : "low");
            return `<button type="button" data-detail-column="${escapeHtml(item.columnName)}" class="is-${level} ${item.columnName === selectedColumnName ? "is-active" : ""}"><i></i><span><b>${escapeHtml(item.columnName)}</b><small>${escapeHtml(item.columnComment || item.dataType || "-")}</small></span><em>${escapeHtml(formatNumber(score))}</em></button>`;
    ...
    function renderSelectedColumn() {
        ...
        if (!state.selectedColumnName) {
            state.root.querySelector("[role='dialog']")?.setAttribute("aria-busy", "false");
            content.innerHTML = `
                <div class="descriptive-statistics-state is-empty">
        ...
        content.querySelectorAll("[data-detail-column]").forEach((button) => {
            button.addEventListener("click", () => {
                state.selectedColumnName = button.dataset.detailColumn || column.columnName;
                const select = state.root?.querySelector("#descriptiveStatisticsColumn");
                if (select) select.value = state.selectedColumnName;
                renderSelectedColumn();
            });
