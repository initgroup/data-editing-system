...
- 먼저 [QUICK_EDIT_RELIABILITY.md](QUICK_EDIT_RELIABILITY.md)의 장애 원인과 검증 기준을 확인합니다.
- FLOW 단계나 기본 모델을 추가하면 코드뿐 아니라 배포 프리셋, 기존 DB의 미등록/일부 등록/비활성 상태, 저장된 JOB, 퀵 에디팅 재시도를 함께 점검합니다. 모델 준비 함수를 mock으로 통째로 대체한 테스트만으로 자동 실행 검증을 완료했다고 판단하지 않습니다.
- `scripts/test_offline.py`로 `.env` 로드와 실제 Oracle/네트워크 연결을 차단한 회귀 검사를 실행합니다. 실제 Oracle 적용을 검증하지 않았다면 그 한계를 결과에 명시합니다.
- 등록이 없다는 이유로 관리자 모델 등록을 임의 생성하거나 비활성 설정을 덮어쓰지 않습니다. 내장 기본값 재사용은 서버의 명시적인 허용 목록과 저장된 JOB 계약으로 제한합니다.
- SQL 변경은 설치 DDL과 실제 조회 범위를 함께 검증하고, 수동 실행할 DDL과 서버가 로드하는 조회 SQL을 안내에서 구분합니다.
- 파일 입수 분석은 실제 업로드와 같은 `VARCHAR2` 컬럼·문자열 데이터로 검증합니다. 숫자형 합성 테이블만으로 완료하지 않습니다. 숫자 변환 정책은 학습·탐지 SQL·실시간 조회·최종 에디팅에서 일치해야 하며, 퀵/상세 화면 모두 생성된 결과와 0건 사유를 확인합니다.
- 수식 발굴은 수식 건수나 높은 R²만으로 완료를 판단하지 않습니다. 작은 구성항·상쇄 관계·상관이 높은 대리변수가 있는 합·차 자료에서 실제 구성항을 찾는지 검사합니다. 단순화한 식은 표시 문자열뿐 아니라 저장 AST·SQL·최종 수정 검증에도 동일하게 적용하고, 무관 자료의 미발굴과 검증 자료로 계수를 재학습하지 않는지도 확인합니다.
